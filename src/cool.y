...
%{
  ...
  for the location of tokens */
    ...
    */
    ...
    feature_list : /* no methods */ {  $$ = nil_Features(); }
    ...
    method_expr_list : expression { $$ = single_Expressions($1); }
                     | method_expr_list ',' expression { $$ = append_Expressions($1, single_Expressions($3)); }
    ;
    ...
    let_expr : OBJECTID ':' TYPEID IN expression %prec LET { $$ = let($1, $3, no_expr(), $5); }
             | OBJECTID ':' TYPEID ASSIGN expression IN expression %prec LET { $$ = let($1, $3, $5, $7); }
             | OBJECTID ':' TYPEID ',' let_expr { $$ = let($1, $3, no_expr(), $5); }
             | OBJECTID ':' TYPEID ASSIGN expression ',' let_expr { $$ = let($1, $3, $5, $7); }
             | error ',' let_expr { }
    ;
             

    expression : OBJECTID ASSIGN expression { $$ = assign($1, $3); }
               | expression '.' OBJECTID '(' method_expr_list ')' { $$ = dispatch($1, $3, $5); }
               | expression '.' OBJECTID '(' ')' { $$ = dispatch($1, $3, nil_Expressions()); }
               | expression '@' TYPEID '.' OBJECTID '(' method_expr_list ')' { $$ = static_dispatch($1, $3, $5, $7); }
               | expression '@' TYPEID '.' OBJECTID '(' ')' { $$ = static_dispatch($1, $3, $5, nil_Expressions()); }
               | OBJECTID '(' method_expr_list ')' { $$ = dispatch(object(idtable.add_string("self")), $1, $3); } 
